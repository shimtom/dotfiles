...
setup_git() {
    ...
    if [[ "$(uname)" == "Darwin" ]]; then
        git config -f ~/.gitconfig_local credential.helper "osxkeychain"
    else
        read -rn 1 -p "Save user and password to an unencrypted file to avoid writing? [y/N]: " save
        if [[ $save =~ ^([Yy])$ ]]; then
            git config -f ~/.gitconfig_local credential.helper "store"
        else
            git config -f ~/.gitconfig_local credential.helper "cache --timeout 3600"
        fi
    fi
}
